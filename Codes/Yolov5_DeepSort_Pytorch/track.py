...
def draw_boxes(img, bbox, identities=None, offset=(0, 0), counting=0, counting_id=[], frame_idx=0):
    center_line = int(img.shape[0] / 2.0)
    mask_flag = False
    print(frame_idx)
    all_mask = []
    try:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs) > 0:
                        ...
                        im0, counting, counting_id = draw_boxes(im0, bbox_xyxy, identities, counting=counting,
                                                                counting_id=counting_id, frame_idx=frame_idx)
                        # to MOT format
